...
function toIndex(page: DocumentRecord): BaseIndex[] {
  ...
  const relatedTopicsHeading = page.structured.headings.find(
    (h) =>
  ...
  page.structured.contents.forEach((p) => {
    ...
    if (
      ...
      heading.id === relatedTopicsHeading.id
    ) {
    ...
    if (heading && !scannedHeadings.has(heading.id)) {
      ...
      const isRootHeading =
        headingLevel === 1 || heading.content === page.title;
      indexes.push(
        createIndex(heading.content, heading.id, heading.content, {
