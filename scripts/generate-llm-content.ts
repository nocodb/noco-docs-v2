...
interface StructuredContent {
  ...
  whySection: string[];
  quickstart: string[];
  docs: { [category: string]: { title: string; url: string; description: string }[] };
...
class LLMContentGenerator {
  ...
  async generateLLMContent() {
    ...
    const structuredContent = await this.generateStructuredContent(processedUrls);
  ...
  private async extractMetaFromFile(filePath: string, buildDir: string): Promise<SitemapUrl | null> {
    try {
  ...
  private categorizeUrl(url: string): string {
    ...
    if (path.includes('/docs/scripts')) return 'scripts';
    if (path.includes('/docs/self-hosting')) return 'self-hosting';
    if (path.includes('/docs/product-docs')) return 'product-docs';
    if (path.includes('/docs/changelog')) return 'changelog';
    if (path.includes('/docs') && !path.includes('/docs/')) return 'docs-overview';
    ...
    if (path.includes('/getting-started') || path.includes('/quickstart')) return 'quickstart';
    if (path.includes('/install') || path.includes('/setup')) return 'installation';
    if (path.includes('/feature') || path.includes('/capability')) return 'features';
  ...
  private getDocCategory(url: string): string {
    ...
    if (path.includes('/docs/scripts')) return 'Scripts';
    if (path.includes('/docs/self-hosting')) return 'Self-hosting';
    if (path.includes('/docs/product-docs/getting-started')) return 'Getting Started';
    if (path.includes('/docs/product-docs/bases')) return 'Bases';
    if (path.includes('/docs/product-docs/tables')) return 'Tables';
    if (path.includes('/docs/product-docs/fields')) return 'Fields';
    if (path.includes('/docs/product-docs/records')) return 'Records';
    if (path.includes('/docs/product-docs/views')) return 'Views';
    if (path.includes('/docs/product-docs/collaboration')) return 'Collaboration';
    if (path.includes('/docs/product-docs/automation')) return 'Automation';
    if (path.includes('/docs/product-docs/data-sources')) return 'Data Sources';
    if (path.includes('/docs/product-docs/developer-resources')) return 'Developer Resources';
    if (path.includes('/docs/product-docs/extensions')) return 'Extensions';
    if (path.includes('/docs/product-docs/integrations')) return 'Integrations';
    if (path.includes('/docs/product-docs/roles-and-permissions')) return 'Roles & Permissions';
    if (path.includes('/docs/product-docs/account-settings')) return 'Account Settings';
    if (path.includes('/docs/product-docs/workspaces')) return 'Workspaces';
    if (path.includes('/docs/product-docs/table-operations')) return 'Table Operations';
    if (path.includes('/docs/product-docs/table-details')) return 'Table Details';
    if (path.includes('/docs/product-docs/engineering')) return 'Engineering';
    if (path.includes('/docs/product-docs/cloud-enterprise-edition')) return 'Cloud & Enterprise';
    if (path.includes('/docs/product-docs')) return 'Product Documentation';
    if (path.includes('/docs/changelog')) return 'Changelog';
    if (path.includes('/docs') && !path.includes('/docs/')) return 'Overview';
    
    return 'Other';
  }

  private groupDocsByCategory(urls: SitemapUrl[]): { [category: string]: { title: string; url: string; description: string }[] } {
    const grouped: { [category: string]: { title: string; url: string; description: string }[] } = {};
  ...
  private async generateStructuredContent(urls: SitemapUrl[]): Promise<StructuredContent> {
    // Group docs by categories - filter for documentation URLs
    const docUrls = urls.filter(url => 
      url.url.includes('/docs/') || url.category === 'docs' || url.category === 'documentation'
    );
    ...
    const quickstartPages = urls.filter(url => 
    ...
    const featurePages = urls.filter(url => 
    ...
    const homepage = urls.find(url => url.url === this.siteUrl || url.url === `${this.siteUrl}/`);
  ...
  private extractTagline(homepage?: SitemapUrl): string {
    if (homepage) {
      const description = homepage.metaDescription || homepage.ogDescription || '';
      if (description) {
        return description;
      }
    }
  ...
  private generateQuickstartFromMeta(pages: SitemapUrl[]): string[] {
    ...
    for (const page of pages) {
      ...
      if (description) {
        ...
        if (description.toLowerCase().includes('install') || description.toLowerCase().includes('setup')) {
          steps.push(`Install NocoDB - ${description}`);
        } else if (description.toLowerCase().includes('connect') || description.toLowerCase().includes('database')) {
          steps.push(`Connect your database - ${description}`);
        } else if (description.toLowerCase().includes('start') || description.toLowerCase().includes('begin')) {
  ...
  private generateWhySectionFromMeta(pages: SitemapUrl[]): string[] {
    ...
    for (const page of pages) {
      ...
      if (description && description.length > 30) {
        ...
        benefit = benefit.replace(/^(Learn|Discover|Explore|Find out|See how)/i, '');
