...
await statusField.updateOptionsAsync({
  choices: [
    ...
    { title: 'Under Review', color: 'orange' },
...
// Update its name
await oldField.updateNameAsync('Project Manager');

output.text('Field name updated successfully.');
```
...
await projectsTable.createFieldAsync({
  ...
  options: {
    choices: [
      ...
      { title: 'Enhancement', color: 'purple' }
    ]
  }
});
```
...
await tasksTable.createFieldAsync({
  ...
  options: {
    relation_type: 'bt', // bt = Belongs To (many-to-one)
...
await tasksTable.createFieldAsync({
  ...
  type: UITypes.Lookup,
  options: {
    relation_field_id: projectField.id, // The link field
    lookup_field_id: projectsTable.getField('Project Manager').id // The field to look up
  }
});
```
...
await projectsTable.createFieldAsync({
  ...
  type: UITypes.Rollup,
  options: {
    relation_field_id: tasksField.id, // The link field
    rollup_field_id: estimatedHoursField.id, // The field to roll up
...
await projectsTable.createFieldAsync({
  ...
  options: {
    formula: 'DATETIME_DIFF(TODAY(), {Due Date}, "days")',
    result: {
      type: UITypes.Number,
      format: {
...
Creates interactive buttons that can trigger actions.
...
await projectsTable.createFieldAsync({
  ...
  options: {
    ...
    color: 'blue',
    theme: 'solid',
    formula: 'mailto:{Assigned To.email}?subject=Reminder: {Task Name}&body=This is a reminder that the task "{Task Name}" is due on {Due Date}.'
  }
});
```
...
const updatedChoices = [
  ...currentChoices,
  { title: 'New Category', color: 'purple' }
];
