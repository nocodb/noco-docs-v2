...
(async () => {
    ...
    // TableID
    // TableID can be retrieved from the URL (OR) by using table context menu
    const tableID = 'YOUR_TABLE_ID';

    // Read the image file
    const photoStream = fs.createReadStream('./uploads/4/1mb.pdf');

    // Create form data
    const formData = new FormData();
    formData.append('file', photoStream);
    // formData.append('url', 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf');
    // Define headers for uploading the file
    const uploadHeaders = {
        'xc-token': apiToken,
        body: {
