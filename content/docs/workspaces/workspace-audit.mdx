...
<Callout type="info">Workspace audit logs are available only in **Enterprise Edition** in both self-hosted & cloud-hosted plans</Callout>
...
⚠️ Note: This section **excludes** record-level audit logs (such as changes made to individual records). To view those, open a record in expanded view and navigate to the Audit tab. Refer [here](/docs/product-docs/records/expand-record#record-audit) for more details.
