...
<Callout type="info"> NocoDB recommends using **MySQL version 5.7 or later** and **PostgreSQL version 14 or later** for best compatibility and performance. </Callout>
...
<Callout type="info">Option (4b) to create a new connection is available only if the user has the required permissions to create new connection (Workspace Creator+)</Callout>
...
<Callout type="info">Data editing can only be disabled when schema editing is also disabled</Callout>
