...
<Callout type="info">To change database connection configuration parameters (like host, port, and such) use [Edit connection](/docs/product-docs/integrations/actions-on-connection#edit) option. Edit connection option is available only for the users with Workspace Creator+ permissions</Callout>
