...
- The Self hosted version of NocoDB (`Instance-1`) is accessible.
...
4. The URL will be copied to your clipboard. The modal will now enter **listening mode**, waiting for a migration request from the Self hosted instance.
...
On the Self hosted instance (**Instance-1**):
...
Once the migration is triggered, a status popup will appear on the self-hosted OSS instance (Instance-1). Upon successful completion, a confirmation message will be displayed on the Enterprise Edition (Instance-2).
