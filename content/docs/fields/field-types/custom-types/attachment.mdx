...
Note:
- You can configure the maximum file size and number of attachments allowed per cell using environment variables in self-hosted version. Refer to [Environment variables](#environment-variables) section for more details.
- In Cloud version, the maximum file size allowed is 128 MiB and maximum number of attachments allowed per cell is 1000.
...
![image](/img/v2/fields/types/attachment/cell-display.png)
...
All supported environment variables are described [here](/docs/self-hosting/environment-variables).

## Related articles
- [Attaching a file from mobile](/docs/product-docs/views/view-types/form#attaching-a-file-from-mobile-device)
