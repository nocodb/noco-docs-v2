...
- `Has-Many` - A record in one table can have multiple records in another table.
  - Example: **Country** has-many **Cities**.
...
- `One-to-One` - A record in one table can have only a single record in another table.
...
3. Select **Links** from the list of field types.
4. Select the relation type : `Many-to-Many`, `Has-Many`, OR `One-to-one`.
5. Select the table to which the relation is to be established with.
6. **Limit Record Selection to a vScreenshot 2025-12-09 at 3.47.25 PMiew**: (Optional) Enable this option to restrict the records available for selection in the `Link records` modal to a specific view in the related table. [Read more](#limit-record-selection)
7. **Limit Record Selection to filters**: (Optional, Cloud only) Enable this option to restrict the records available for selection in the `Link records` modal based on specific filter conditions. [Read more](#limit-record-selection)
8. **Advance settings**: Configure label to be used for display in the cell. Defaults to `Link`, `Links` (Optional).
9. Click on `Save Field` button.

![image](/img/v2/fields/types/links/add-field.png)
...
![image](/img/v2/fields/types/links/cell-display.png)
...
<Callout type="info">Multiple records can be linked at once</Callout>

### Unlink records

To unlink a record, open the `Linked Records` modal & click on the `-` icon on the card of the record to be unlinked.

<Callout type="info">Multiple records can be unlinked at once</Callout>
