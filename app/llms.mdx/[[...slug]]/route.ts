...
export function generateStaticParams() {
  return [
    ...source
      .generateParams()
      .map((param) => ({ slug: ["product-docs", ...(param.slug || [])] })),
    ...scriptsSource
      .generateParams()
      .map((param) => ({ slug: ["scripts", ...(param.slug || [])] })),
    ...selfHostingSource
      .generateParams()
      .map((param) => ({ slug: ["self-hosting", ...(param.slug || [])] })),
    ...changelogSource
      .generateParams()
      .map((param) => ({ slug: ["changelog", ...(param.slug || [])] })),
    ...legalDocsSource
      .generateParams()
      .map((param) => ({ slug: ["legal", ...(param.slug || [])] })),
  ];
}
